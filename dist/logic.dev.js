...
var varea, tracoBe, genericoSap, nrPilar, vl, nv, btn, btn4, cont, cont1, nrSapatas, btn1, btn2, btn3, res, com, selT, selT1, lar, es, esAr, traco, nrA, tracoReb, vltraco, vlArg, massa, espReb, rebfinal, blocoMetro, metro, larS, comS, altS, sapata, sapataVl, cP, lP, aP, cV, lV, aV, trV, nrV;
...
function validacoes(ac) {
  if (ac.value == "" || _typeof(ac.value) == NaN) alert("inroduza um Numero");
}
...
function eventos() {
  ...
  btn3.addEventListener("click", function () {
    ...
    if (vl < cont) {
      nrdSapatas(vl, selT, comS.value, larS.value, altS.value); // zerar();

      comS.value.innerHTML = "";
      larS.value.innerHTML = "";
      altS.value.innerHTML = "";
    } else calculosBet("saida3");
  });
...
function calcularArea() {
  ...
  if (isNaN(nrA) == true) document.getElementById("saida").innerHTML = " Introduza um Numero " + nrA;else document.getElementById("saida").innerHTML = nrA + " Blocos";
}
...
function nrdSapatas(n, va, x, y, z) {
  genericoSap[vl] = calculosBetao(va, x, y, z); // zerar();
